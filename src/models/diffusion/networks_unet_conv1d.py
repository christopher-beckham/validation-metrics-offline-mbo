...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, groups=8):
        super().__init__()
        self.mlp = (
            nn.Sequential(nn.SiLU(), nn.Linear(time_emb_dim+1, dim_out*2))
    ...
    def forward(self, x, y, time_emb):
        ...
        if exists(self.mlp) and exists(time_emb):
            time_emb = self.mlp(torch.cat((time_emb, y), dim=1))
...
class UNet_Conv1d(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv1d(1, init_dim, 7, padding=3) #channels*2
    ...
    def forward(self, x, y, time):
        ...
        # For convenience, pad to the nearest power of two
        orig_length = x.size(-1)
        # 5184 = 4096+1024+64
        pad_length = 5184 - orig_length
        padding = torch.ones(x.size(0), pad_length).to(x.device)

        x = torch.cat((x, padding), dim=1)
        
        x = x.unsqueeze(1) # from (bs, p) to (bs, 1, p)
        ...
        #print(self.final_block(x,y,t).shape)

        x_final = self.final_conv(self.final_block(x,y,t))

        #print(">", x_final.shape)

        assert x_final.size(1)==1

        return x_final.squeeze(1)[:, 0:orig_length]
